...
function CemberinAlani(yaricap, pi) {
  return parseInt(pi * Math.pow(yaricap, 2));
}
...
for (let i = 0; i < sayilar.length; i++) {
  if (sayacNesne[sayilar[i]]) {
    sayacNesne[sayilar[i]]++;
  } else {
    sayacNesne[sayilar[i]] = 1;
  }
}

for (let sayi in sayacNesne) {
  if (sayacNesne[sayi] > 1) {
    tekraredensayilar.push(
      `${sayi} sayısı ${sayacNesne[sayi]} kere tekrar edilmiştir`
    );
  }
}
